(() => {
  ...
  function pageImageMarkup(shot){
    const src=shot?.src||shot?.image;if(!src)return '';
    const alt=shot.alt||shot.title||'Dragonwilds Sync screenshot';
    return `<button class="helpy-screenshot ready" type="button" data-helpy-image="${esc(imageUrl(src))}" data-helpy-alt="${esc(alt)}"><img src="${esc(imageUrl(src))}" alt="${esc(alt)}" loading="lazy"><span><strong>${esc(shot.title||alt)}</strong>${shot.description?`<small>${esc(shot.description)}</small>`:''}</span></button>`;
  }
  function renderPage(page){
    const planned=(page.screenshots||[]).filter((shot)=>shot.image||!(page.images||[]).length);
    const gallery=[...(page.images||[]).map(pageImageMarkup),...planned.map(screenshotMarkup)].filter(Boolean);
    const screenshots=gallery.length?`<section class="helpy-screenshots"><div class="helpy-screenshots-heading"><div><div class="eyebrow">Current application</div><h3>Visual walkthrough</h3></div><small>Captures use demonstration profiles with credentials, addresses, tokens, and personal paths removed.</small></div><div class="helpy-screenshot-grid">${gallery.join('')}</div></section>`:'';
    const meta=[page.audience,page.time].filter(Boolean).map((value)=>`<span>${esc(value)}</span>`).join('');
    article.innerHTML=`<header><div class="eyebrow">${esc(page.category||'Guide')}</div><h2>${esc(page.title)}</h2>${meta?`<div class="helpy-page-meta">${meta}</div>`:''}<p class="helpy-article-intro">${esc(page.intro||page.summary||'')}</p></header>${page.callout?`<aside class="helpy-callout"><strong>${esc(page.callout.title||'Important')}</strong><p>${esc(page.callout.body||'')}</p></aside>`:''}${page.image?`<button class="helpy-article-shot" type="button" data-helpy-image="${esc(imageUrl(page.image))}" data-helpy-alt="${esc(page.image_alt||page.title)}"><img src="${esc(imageUrl(page.image))}" alt="${esc(page.image_alt||page.title)}" loading="eager"></button>`:''}<div class="helpy-section-grid">${(page.sections||[]).map((section)=>`<section class="helpy-section ${esc(section.kind||'')}"><h3>${esc(section.title)}</h3>${section.body?`<p>${esc(section.body)}</p>`:''}${section.steps?.length?`<ol>${section.steps.map((step)=>`<li>${esc(step)}</li>`).join('')}</ol>`:''}</section>`).join('')}</div>${screenshots}${page.tips?.length?`<div class="helpy-tips">${page.tips.map((tip)=>`<span>${esc(tip)}</span>`).join('')}</div>`:''}`;
